...
class AdbOperationImpl implements IOperation {
//    KEYCODE列表
//            电话键
//    键名 描述 键值
//
...
//    KEYCODE_UNKNOWN 未知按键
    ...
    public void move(int startX, int startY, int endX, int endY, int time) {
        exec(String.format("input swipe %s %s %s %s %s", startX, startY, endX, endY, time));
    }

    @Override
    public void click(int x, int y) {
        exec(String.format("input tap %s %s", x, y));
    }
    ...
    public void text(String text) {
        exec(String.format("input text %s", text));
    }
