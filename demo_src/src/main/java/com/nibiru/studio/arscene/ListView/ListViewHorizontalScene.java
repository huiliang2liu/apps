...
/**
 * 演示ListView列表控件，支持横向布局
 */

/**
 * Show ListView list control, supporting horizontal layout
 */

public class ListViewHorizontalScene extends BaseScene implements IXActorEventListener {
    private List<String> datas = new ArrayList<>();
    private MyAdapter adapter;
    private XToast toast;

    public ListViewHorizontalScene() {
    ...
    public boolean onGazeTrigger(XActor actor) {
        if ("add".equals(actor.getName())) {
            ...
            synchronized (ListViewHorizontalScene.this) {
                for (int i = 0; i < 4; i++) {
                    datas.add("hahaha");
                }
            }
    ...
    private class MyAdapter extends XItemAdapter {
        ...
        @Override
        public int getXActorType(int position) {
            if (position % 2 == 0) {
                return 1;
            } else {
                return 2;
        ...
        public XItem getXItem(int position, XItem convertItem, XActor parent) {
            String data;
            synchronized (ListViewHorizontalScene.class) {
            ...
            if (convertItem == null) {
                ...
                if (position % 2 == 0) {
                    convertItem.setSize(0.2f, 0.25f);
                } else {
                    convertItem.setSize(0.2f, 0.25f);
                }
                ...
                image.setLayoutParam(new XActorGroup.LayoutParam(0, 0.03f, 0.01f, 3));
                image.setName("image");
                image.setSize(0.1f, 0.1f);
                XLabel label = new XLabel("文本：" + data);
                label.setLayoutParam(new XActorGroup.LayoutParam(0, -0.05f, 0.01f, 3));
                label.setName("title");
                label.setSize(0.15f, 0.03f);
                ...
                convertItem.setEventListener(new IXActorEventListener() {
                    ...
                    public boolean onGazeTrigger(XActor actor) {
                        ...
                        toast = XToast.makeToast(ListViewHorizontalScene.this, "当前选中：" + ((XLabel)finalConvertItem.getChild("title")).getTextContent(), 0xffffffff, 3000);
                        toast.setGravity(0.0f, -0.2f, CalculateUtils.CENTER_Z + 0.01f);
    ...
    public void onCreate() {
        ...
        actorListView = new XActorListView(XActorListView.ListViewType.Horizontal);
        ...
        //设置尺寸
        //Set size
        actorListView.setSize(0.6f, 0.4f);
    ...
    void initTopActor() {
        ...
        buttonAdd.setCenterPosition(-0.25f, 0.2f, CalculateUtils.CENTER_Z);
        ...
        buttonJump.setCenterPosition(0.25f, 0.2f, CalculateUtils.CENTER_Z);
